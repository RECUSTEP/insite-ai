...
import { saveHelpAction } from "../_actions/save-help";

type Schema = z.infer<typeof helpSchema>;
type Props = {
...
const fieldConfigs: FieldConfig[] = [
  ...
  {
    label: "Meta インサイト（Instagram・Threads）",
    key: "meta-insight",
    section: "分析AI",
  },
  ...
  { label: "Threads（画像あり）", key: "threads", section: "ライティングAI（Threads）" },
  { label: "Threads（画像なし）", key: "threads-no-image", section: "ライティングAI（Threads）" },
  { label: "画像あり", key: "google-map", section: "ライティングAI（Google Map）" },
  { label: "画像なし", key: "google-map-no-image", section: "ライティングAI（Google Map）" },
];

export function SaveHelpForm({ defaultValue }: Props) {
  const [lastResult, formAction] = useFormState(saveHelpAction, {});
  const [form, fields] = useForm({
    defaultValue,
    lastResult,
    onValidate({ formData }) {
      return parseWithZod(formData, { schema: helpSchema });
    },
